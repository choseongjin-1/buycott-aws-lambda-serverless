              ...
              , A.STORE_SRNO AS storeSrno
                  , A.USER_SRNO AS userSrno
                      , A.REVIEW_CONTENT AS reviewContent
                          , A.REG_DT AS regDt
                              , (SELECT CEIL(COUNT(*) / ?) FROM TB_REVIEW WHERE STORE_SRNO = A.STORE_SRNO) AS totalPageNum
                                                                ...
                                                                AND A.STORE_SRNO = ?
                                                                  AND A.VALID_YN = 'Y'
                                                                    ORDER BY A.REG_DT DESC
                                                                      LIMIT ? OFFSET ?
                                                                        `
    const result = await common.runQuery(connection, query, [limit, event.storeSrno, limit, offset]);
    return result
}
